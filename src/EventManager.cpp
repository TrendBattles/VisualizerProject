...
void EventManager::init() {
    ...
    //Event buttons
    eventButtons[0] = createRectangle(Vector2{0.0f, (float) GetScreenHeight() - 60.0f}, Vector2{100.0f, (float) GetScreenHeight()}, 5.0f, 0);
    eventButtons[0].setColor(Color {170, 170, 170, 255}, BLUE);
    eventButtons[0].setText(createText("Insert", GetFontDefault(), 20.0f, 5.0f, Vector2Lerp(eventButtons[0].startPosition, eventButtons[0].endPosition, 0.5f), RED));
    eventButtons[0].setTextCenter(true);

    eventButtons[1] = createRectangle(Vector2{240.0f, (float) GetScreenHeight() - 60.0f}, Vector2{340.0f, (float) GetScreenHeight()}, 5.0f, 0);
    eventButtons[1].setColor(Color {170, 170, 170, 255}, BLUE);
    eventButtons[1].setText(createText("Remove", GetFontDefault(), 20.0f, 5.0f, Vector2Lerp(eventButtons[1].startPosition, eventButtons[1].endPosition, 0.5f), RED));
    eventButtons[1].setTextCenter(true);

    eventButtons[2].isHidden = true;

    //Event Text Box
    eventTextBox[0] = createRectangle(Vector2{120.0f, (float) GetScreenHeight() - 60.0f}, Vector2{220.0f, (float) GetScreenHeight()}, 5.0f, 0);
    eventTextBox[0] = createRectangle(Vector2{120.0f, (float) GetScreenHeight() - 60.0f}, Vector2{220.0f, (float) GetScreenHeight()}, 5.0f, 0);
    eventTextBox[0].setColor(Color {170, 170, 170, 255}, BLUE);
    eventTextBox[0].setText(createText(std::string(), GetFontDefault(), 20.0f, 2.0f, Vector2Lerp(eventTextBox[0].startPosition, eventTextBox[0].endPosition, 0.5f), BLACK));
    eventTextBox[0].setTextCenter(true);
    
    eventTextBox[1] = createRectangle(Vector2{360.0f, (float) GetScreenHeight() - 60.0f}, Vector2{460.0f, (float) GetScreenHeight()}, 5.0f, 0);
    eventTextBox[1] = createRectangle(Vector2{360.0f, (float) GetScreenHeight() - 60.0f}, Vector2{460.0f, (float) GetScreenHeight()}, 5.0f, 0);
    eventTextBox[1].setColor(Color {170, 170, 170, 255}, BLUE);
    eventTextBox[1].setText(createText(std::string(), GetFontDefault(), 20.0f, 2.0f, Vector2Lerp(eventTextBox[1].startPosition, eventTextBox[1].endPosition, 0.5f), BLACK));
...
bool EventManager::isHidden(int idx) {
    return idx < 0 || idx >= getSize() ? true : eventButtons[idx].isHidden;
}
bool EventManager::isHidden(std::string message) {
    return isHidden(find(eventButtonsID.begin(), eventButtonsID.end(), message) - eventButtonsID.begin());
}
